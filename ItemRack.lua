...
-- Many addon management APIs were moved to the C_AddOns namespace in newer clients
...
-- TBC Anniversary Edition uses WOW_PROJECT_ID = 5
-- It's separate from the old WOW_PROJECT_BURNING_CRUSADE_CLASSIC (5 in old, 2 in some versions)
...
function ItemRack.IsBCC()
	-- TBC Anniversary uses project ID 5, handle both old and new project IDs
	return WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or WOW_PROJECT_ID == 5
end

function ItemRack.IsTBCAnniversary()
	-- Specific check for TBC Anniversary Edition (2025+)
	-- Interface version 20505+ and project ID 5
	return WOW_PROJECT_ID == 5 and wowtoc >= 20505
end
...
-- Safe wrapper to get number of talent specs (handles API differences across versions)
function ItemRack.GetNumSpecs()
	-- Try GetNumSpecializations first (seen in API list)
	if GetNumSpecializations then
		local ok, result = pcall(GetNumSpecializations)
		if ok and result then
			return result
		end
	end
	-- Try GetNumTalentGroups (WotLK style)
	if GetNumTalentGroups then
		local ok, result = pcall(GetNumTalentGroups)
		if ok and result then
			return result
		end
	end
	-- Try GetNumSpecGroups (Cata+ style)
	if GetNumSpecGroups then
		local ok, result = pcall(GetNumSpecGroups)
		if ok and result then
			return result
		end
	end
	-- Fallback: if GetActiveTalentGroup exists, assume 2 specs
	...
	-- TBC Anniversary fallback: Check if PlayerSpecTab2 exists (dual spec purchased)
	if PlayerSpecTab2 or _G["PlayerSpecTab2"] then
		return 2
	end
	-- Final fallback: if talent frame can be loaded, check for spec tabs
	if not PlayerTalentFrame then
		if C_AddOns and C_AddOns.LoadAddOn then
			pcall(C_AddOns.LoadAddOn, "Blizzard_TalentUI")
...
-- Safe wrapper to get active spec (handles API differences across versions)
function ItemRack.GetActiveSpec()
	-- Try GetActiveTalentGroup first (seen in API list)
	if GetActiveTalentGroup then
		local ok, result = pcall(GetActiveTalentGroup)
		if ok and result then
			return result
	...
	-- Try GetActiveSpecGroup (Cata+ style)
	if GetActiveSpecGroup then
		local ok, result = pcall(GetActiveSpecGroup)
		if ok and result then
			return result
		end
	end
...
-- Safe wrapper to set active spec (handles API differences across versions)
function ItemRack.SetActiveSpec(specIndex)
	-- Try SetActiveTalentGroup first (WotLK style API)
	if SetActiveTalentGroup then
		local ok = pcall(SetActiveTalentGroup, specIndex)
		if ok then
			return true
		end
	end
	-- Try SetActiveSpecGroup (Cata+ style API)
	if SetActiveSpecGroup then
		local ok = pcall(SetActiveSpecGroup, specIndex)
		if ok then
			return true
		end
	end
	
	-- TBC Anniversary Edition - must use UI method
	-- Step 1: Load the talent UI addon if not loaded
	if not PlayerTalentFrame then
		if C_AddOns and C_AddOns.LoadAddOn then
	...
	if not PlayerTalentFrame then
		ItemRack.Print("Error: Could not load talent UI")
		return false
	end
	
	-- Step 2: Open the talent frame
	ShowUIPanel(PlayerTalentFrame)
	
	-- Step 3: Click spec tab, then activate, then close (with delays)
	C_Timer.After(0.1, function()
		-- Click the spec tab
		local specTab = _G["PlayerSpecTab"..specIndex]
		if specTab then
			specTab:Click()
		end
		
		-- Step 4: Click activate button after tab registers
		C_Timer.After(0.2, function()
			if PlayerTalentFrameActivateButton and PlayerTalentFrameActivateButton:IsEnabled() then
				PlayerTalentFrameActivateButton:Click()
			end
			
			-- Step 5: Close the talent frame
...
-- Change talent spec when equipping a set (for dual spec support)
function ItemRack.ChangeSpecForSet(targetSpec, setname)
	-- Can't change specs in combat
	if InCombatLockdown() then
		ItemRack.Print("Cannot change spec while in combat. Gear will be equipped, but spec change skipped.")
		return
	end
...
function ItemRack.CreateTimer(name,func,delay,rep)
	-- Initialize TimerPool if it doesn't exist yet (Options OnLoad can call this before InitTimers)
	if not ItemRack.TimerPool then
		ItemRack.TimerPool = {}
	end
	if not ItemRack.Timers then
		ItemRack.Timers = {}
	end
...
function ItemRack.IsTimerActive(name)
	-- Guard: Timers may not exist yet
	if not ItemRack.Timers then
		return nil
	end
...
function ItemRack.StartTimer(name,delay)
	-- Guard: TimerPool or the specific timer may not exist yet
	if not ItemRack.TimerPool or not ItemRack.TimerPool[name] then
		return
	end
...
function ItemRack.StopTimer(name)
	-- Guard: TimerPool may not exist yet
	if not ItemRack.TimerPool or not ItemRack.Timers then
		return
	end
...
function ItemRack.OnUpdate(self,elapsed)
	-- Guard: Timers and TimerPool may not exist yet
	if not ItemRack.Timers or not ItemRack.TimerPool then
		return
	end
	local timerPool
...
--[[ Options support (integrated module) ]]

function ItemRack.ToggleOptions(self,tab)
	-- Options module is now integrated, no need to load it
...
function ItemRack.SlashHandler(arg1)
	...
	elseif string.match(arg1,"^spec ?(%d*)$") then
		local specNum = tonumber(string.match(arg1,"^spec ?(%d*)$"))
		if specNum and specNum >= 1 and specNum <= 3 then
			ItemRack.Print("Attempting to switch to spec "..specNum)
			ItemRack.SetActiveSpec(specNum)
		else
			ItemRack.Print("Usage: /itemrack spec 1 or /itemrack spec 2")
			ItemRack.Print("Current spec: "..(ItemRack.GetActiveSpec() or "unknown"))
			ItemRack.Print("Number of specs: "..ItemRack.GetNumSpecs())
		end
	elseif arg1=="debugspec" then
		-- Debug command to show spec frame info
		ItemRack.Print("=== Spec Debug Info ===")
		ItemRack.Print("GetActiveSpec: "..(ItemRack.GetActiveSpec() or "nil"))
		ItemRack.Print("GetNumSpecs: "..ItemRack.GetNumSpecs())
		ItemRack.Print("PlayerTalentFrame: "..(PlayerTalentFrame and "exists" or "nil"))
		ItemRack.Print("PlayerTalentFrameActivateButton: "..(PlayerTalentFrameActivateButton and "exists" or "nil"))
		for i=1,3 do
			local tab = _G["PlayerSpecTab"..i]
			ItemRack.Print("PlayerSpecTab"..i..": "..(tab and (tab:IsShown() and "shown" or "exists but hidden") or "nil"))
		end
	else
		...
		ItemRack.Print("/itemrack spec 1|2 : switch to spec 1 or 2.")
