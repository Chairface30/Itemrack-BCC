...
function ItemRackOpt.OnLoad(self)
	...
	ItemRackOpt.InitializeSpecDropdown()
end

-- Spec dropdown for dual spec feature
function ItemRackOpt.InitializeSpecDropdown()
	if not ItemRackOptSetsSpecFrame then return end
	
	UIDropDownMenu_SetWidth(ItemRackOptSetsSpecFrame, 100)
	UIDropDownMenu_Initialize(ItemRackOptSetsSpecFrame, ItemRackOpt.SpecDropdown_Initialize)
	UIDropDownMenu_SetSelectedValue(ItemRackOptSetsSpecFrame, 0)
	UIDropDownMenu_DisableDropDown(ItemRackOptSetsSpecFrame)
end

function ItemRackOpt.SpecDropdown_Initialize()
	local info = UIDropDownMenu_CreateInfo()
	
	-- "None" option - don't change spec
	info.text = "Don't change"
	info.value = 0
	info.func = ItemRackOpt.SpecDropdown_OnClick
	info.checked = (UIDropDownMenu_GetSelectedValue(ItemRackOptSetsSpecFrame) == 0)
	UIDropDownMenu_AddButton(info)
	
	-- Always show both spec options for TBC Anniversary dual spec
	info.text = "Spec 1 (Primary)"
	info.value = 1
	info.func = ItemRackOpt.SpecDropdown_OnClick
	info.checked = (UIDropDownMenu_GetSelectedValue(ItemRackOptSetsSpecFrame) == 1)
	UIDropDownMenu_AddButton(info)
	
	info.text = "Spec 2 (Secondary)"
	info.value = 2
	info.func = ItemRackOpt.SpecDropdown_OnClick
	info.checked = (UIDropDownMenu_GetSelectedValue(ItemRackOptSetsSpecFrame) == 2)
	UIDropDownMenu_AddButton(info)
end

function ItemRackOpt.SpecDropdown_OnClick(self, arg1, arg2, checked)
	local value = self.value
	UIDropDownMenu_SetSelectedValue(ItemRackOptSetsSpecFrame, value)
	UIDropDownMenu_SetText(ItemRackOptSetsSpecFrame, value == 0 and "Don't change" or ("Spec "..value))
	-- Save spec to current set if one is selected
	local setname = ItemRackOptSetsName:GetText()
	if setname and ItemRackUser.Sets[setname] then
		ItemRackUser.Sets[setname].spec = value > 0 and value or nil
		ItemRack.Print("Set \""..setname.."\" will now use "..(value > 0 and ("Spec "..value) or "no spec change"))
	end
end

function ItemRackOpt.UpdateSpecDropdown(setname)
	if not ItemRackOptSetsSpecFrame then return end
	
	-- Always show the spec dropdown - let user configure it
	-- The actual spec switch will just not happen if dual spec isn't available
	ItemRackOptSetsSpecFrame:Show()
	if ItemRackOptSetsSpecLabelFrame then
		ItemRackOptSetsSpecLabelFrame:Show()
	end
	
	if setname and ItemRackUser.Sets[setname] then
		UIDropDownMenu_EnableDropDown(ItemRackOptSetsSpecFrame)
		local spec = ItemRackUser.Sets[setname].spec or 0
		UIDropDownMenu_SetSelectedValue(ItemRackOptSetsSpecFrame, spec)
		UIDropDownMenu_SetText(ItemRackOptSetsSpecFrame, spec == 0 and "Don't change" or ("Spec "..spec))
	else
		UIDropDownMenu_DisableDropDown(ItemRackOptSetsSpecFrame)
		UIDropDownMenu_SetSelectedValue(ItemRackOptSetsSpecFrame, 0)
		UIDropDownMenu_SetText(ItemRackOptSetsSpecFrame, "Don't change")
	end
end
...
function ItemRackOpt.OnShow(setname)
	-- Guard: Inv and Icons are created in OnLoad, may not exist yet
	if not ItemRackOpt.Inv[0] or not ItemRackOpt.Icons or #ItemRackOpt.Icons == 0 then
		return
	end
...
function ItemRackOpt.SaveSet()
	...
	-- set.equip[0] = nil
	-- set.equip[18] = nil
	-- Save spec setting from dropdown
	if ItemRackOptSetsSpecFrame then
		local specValue = UIDropDownMenu_GetSelectedValue(ItemRackOptSetsSpecFrame)
		set.spec = specValue and specValue > 0 and specValue or nil
	end
...
function ItemRackOpt.ValidateSetButtons()
	...
	if ItemRackUser.Sets[setname] then
		...
		ItemRackOpt.UpdateSpecDropdown(setname)
	else
		-- No set selected, disable spec dropdown
		ItemRackOpt.UpdateSpecDropdown(nil)
	end
end
...
function ItemRackOpt.StartMarquee()
	-- Ensure the SlotMarquee timer exists with a wrapper function
	if not ItemRack.TimerPool or not ItemRack.TimerPool["SlotMarquee"] then
		ItemRack.CreateTimer("SlotMarquee", function() ItemRackOpt.SlotMarquee() end, .1, 1)
	end
	ItemRackOpt.SaveInv()
	for i=0,19 do
		ItemRackOpt.Inv[i].selected = nil
...
function ItemRackOpt.PopulateEventList()
	-- Guard: ItemRackEvents and ItemRackUser.Events may not exist yet during initial load
	if not ItemRackEvents or not ItemRackUser or not ItemRackUser.Events then
		return
	end
	...
	local oldevent = ItemRackOpt.EventSelected and list[ItemRackOpt.EventSelected] and list[ItemRackOpt.EventSelected][1] or nil
	ItemRackOpt.EventSelected = nil
	...
	for i in pairs(events) do
		if user.Set and user.Set[i] then
			setname = user.Set[i]
		elseif events[i].Type=="Script" and user.Enabled and user.Enabled[i] then
			setname = "zzz" -- give it a fake set name for sorting purposes
		else
			setname = nil
		end
		if user.Enabled and user.Enabled[i] and not setname then
			safeToRegister = nil
		end
		table.insert(list,{i,events[i].Type,user.Enabled and user.Enabled[i],setname})
	end
	...
	for i=1,9 do
		...
		if idx<=#(list) then
			...
			if list[idx][2]=="Script" then
				texture = "Interface\\AddOns\\ItemRack\\ItemRackScriptIcon"
